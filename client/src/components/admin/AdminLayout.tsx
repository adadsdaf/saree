      ...
      {pendingOrders.length > 0 || pendingWasalni.length > 0 ? (
        <>
          ...
          {pendingWasalni.slice(0, 5).map((request: any) => (
            ...
            >
              <div className="flex items-start gap-2">
                <div className="w-2 h-2 bg-[#F57C00] rounded-full mt-1.5 flex-shrink-0"></div>
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[#F57C00] mt-0.5">بانتظار المراجعة</p>
                </div>
                <Bike className="h-4 w-4 text-[#F57C00]" />
              </div>
            </div>
          ))}
        </>
